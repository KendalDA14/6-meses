...
function redirectToGate(request, accessToken = "") {
  ...
  url.searchParams.set("locked", "six");
  ...
  if (accessToken) {
    headers.append("Set-Cookie", clearCookie(SIX_ACCESS_COOKIE, request));
  }
...
async function hasValidAccess(request) {
  const accessToken = readCookie(request, SIX_ACCESS_COOKIE);
  const access = await readSignedToken(accessToken);

  return {
    accessToken,
    valid: isValidAccessPayload(access) && access.area === "six",
  };
}

async function serveSixPage() {
  const html = await readFile(new URL("../six_months.html", import.meta.url), "utf8");
...
export default async function handler(request, nodeResponse) {
  ...
  const response = valid ? await serveSixPage() : redirectToGate(request, accessToken);
